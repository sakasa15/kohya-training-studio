...
def list_models():
    ...
    if high_priority:
        ...
        for model_id, info in high_priority:
            print(f"  {model_id:<20} [{info['size']:<10}] {info['filename']}")
            print(f"  └─ {info['description']}")
            print(f"     {info['description_ja']}")
    
    # ⭐ Recommended Models / 推奨モデル
    if medium_priority:
        print("\n⭐ Recommended Models / 推奨モデル (Quality & Specialized / 品質向上・特化用):")
        print("-" * 80)
...
def download_model(model_id: str):
    ...
    if model_id not in AVAILABLE_MODELS:
        ...
        print("'python model_downloader.py list' で利用可能なモデルを確認")
        sys.exit(1)
    ...
    # Check existing file / 既存ファイルチェック
    if dest_path.exists():
        ...
        if response.lower() != 'y':
            ...
            return
        print("Deleting existing file and re-downloading...")
        print("既存ファイルを削除して再ダウンロードします...")
    ...
    try:
        ...
        # Delete incomplete file / 不完全なファイルを削除
        if dest_path.exists():
            try:
                dest_path.unlink()
...
def download_from_url(url: str, filename: str):
    ...
    if not filename.endswith('.safetensors') and not filename.endswith('.ckpt'):
        ...
        print("警告: ファイル名は .safetensors または .ckpt で終わる必要があります")
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  python model_downloader.py list")
        print("  python model_downloader.py download sd15")
        print("  python model_downloader.py download sdxl-base")
        print("  python model_downloader.py url 'https://example.com/model.safetensors' 'my-model.safetensors'")
        print()
        sys.exit(1)
    ...
    if command == "list":
        ...
        if len(sys.argv) < 3:
            ...
            print("Example / 例: python model_downloader.py download sd15")
            sys.exit(1)
        download_model(sys.argv[2])
    
    elif command == "url":
        if len(sys.argv) < 4:
            print("\n❌ Error: URL and filename required / エラー: URLとファイル名が必要です")
            print("Example / 例: python model_downloader.py url 'https://example.com/model.safetensors' 'model.safetensors'")
            sys.exit(1)
